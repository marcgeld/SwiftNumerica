...
let populationCovariance = try Numerica.Statistics.populationCovariance(x, y)
let sampleCovariance = try Numerica.Statistics.sampleCovariance(x, y)
let covarianceAlias = try Numerica.Statistics.covariance(x, y)
let pearsonCorrelation = try Numerica.Statistics.pearsonCorrelation(x, y)
let spearmanCorrelation = try Numerica.Statistics.spearmanCorrelation(x, y)
let correlationAlias = try Numerica.Statistics.correlation(x, y)
let valueStyleCovariance = try x.covariance(with: y)
let valueStyleCorrelation = try x.correlation(with: y)
