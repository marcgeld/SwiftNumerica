...
@Test func outlierAnalysisFindsExtremeValues() throws {
    let analysis = try #require(try Numerica.DataProfiling.outlierAnalysis(.vector([1, 2, 2, 3, 100])))
    #expect(analysis.outliers == [100])
}

@Test func trendAnalysisDetectsIncreasingSeries() throws {
    let analysis = try #require(try Numerica.DataProfiling.trendAnalysis(.vector([1, 2, 3, 4])))
...
@Test func datasetProfilerBuildsSummaryAndCorrelationMatrix() throws {
    ...
    let profile = try DatasetProfiler.profile(tensor)
...
@Test func benfordAnalysisIgnoresNonFiniteAndScalesFractionalValues() throws {
    let analysis = try #require(Numerica.DataProfiling.benfordAnalysis(.vector([.infinity, .nan, 0.04])))
    #expect(analysis.observedFrequencies[4] == 1)
}

@Test func uniformityAnalysisRejectsNonFiniteValues() {
    #expect(Numerica.DataProfiling.uniformityAnalysis(.vector([1, .nan, 3])) == nil)
}
