...
@Test func meanReturnsNilForEmptyTensor() throws {
    try #expect(Numerica.Statistics.mean(.vector([])) == nil)
}
...
@Test func pureSwiftAndAccelerateProduceSameMean() throws {
    ...
    let pureSwift = try #require(try Numerica.Statistics.mean(tensor))
    #expect(pureSwift.isApproximatelyEqual(to: 2.5))

    Numerica.configuration.backend = .accelerate
    let accelerate = try #require(try Numerica.Statistics.mean(tensor))
    #expect(
...
@Test func meanReturnsAverageForNegativeValues() throws {
    let mean = try #require(try Numerica.Statistics.mean(.vector([-2, -4, -6])))
    #expect(mean.isApproximatelyEqual(to: -4))
}

@Test func meanReturnsAverageForDecimalValues() throws {
    let mean = try #require(try Numerica.Statistics.mean(.vector([1.5, 2.5, 3.0])))
    #expect(mean.isApproximatelyEqual(to: 7.0 / 3.0))
}

@Test func medianReturnsMiddleValue() throws {
    let median = try #require(try Numerica.Statistics.median(.vector([5, 1, 3])))
    #expect(median.isApproximatelyEqual(to: 3))
}

@Test func modeReturnsRepeatedValues() throws {
    try #expect(Numerica.Statistics.mode(.vector([1, 2, 2, 3])) == [2])
}

@Test func rangeReturnsMaximumMinusMinimum() throws {
    let range = try #require(try Numerica.Statistics.range(.vector([4, -2, 8, 1])))
...
@Test func sumMinAndMaxSummarizeTensorValues() throws {
    ...
    #expect(try #require(try Numerica.Statistics.sum(tensor)).isApproximatelyEqual(to: 11))
    #expect(try #require(try Numerica.Statistics.min(tensor)).isApproximatelyEqual(to: -2))
    #expect(try #require(try Numerica.Statistics.max(tensor)).isApproximatelyEqual(to: 8))
}

@Test func populationVarianceUsesPopulationDenominator() throws {
    let variance = try #require(
        try Numerica.Statistics.populationVariance(.vector([2, 4, 4, 4, 5, 5, 7, 9])))
    #expect(variance.isApproximatelyEqual(to: 4))
}

@Test func sampleVarianceUsesSampleDenominator() throws {
    let variance = try #require(
        try Numerica.Statistics.sampleVariance(.vector([2, 4, 4, 4, 5, 5, 7, 9])))
    #expect(variance.isApproximatelyEqual(to: 32.0 / 7.0))
    let convenience = try #require(
        try Numerica.Statistics.variance(.vector([2, 4, 4, 4, 5, 5, 7, 9])))
...
@Test func covarianceUsesExpectedDenominator() throws {
    ...
    let population = try #require(try Numerica.Statistics.populationCovariance(x, y))
    let sample = try #require(try Numerica.Statistics.sampleCovariance(x, y))

    #expect(population.isApproximatelyEqual(to: 4.0 / 3.0))
    #expect(sample.isApproximatelyEqual(to: 2))
    #expect(try #require(try Numerica.Statistics.covariance(x, y)).isApproximatelyEqual(to: sample))
    #expect(try #require(try Numerica.Statistics.correlation(x, y)).isApproximatelyEqual(to: 1))
}

@Test func spearmanCorrelationUsesRanks() throws {
    let correlation = try #require(
        try Numerica.Statistics.spearmanCorrelation(.vector([10, 20, 30]), .vector([1, 2, 3])))
    #expect(correlation.isApproximatelyEqual(to: 1))
}

@Test func linearRegressionReturnsSlopeInterceptAndRSquared() throws {
    let result = try #require(
        try Numerica.Statistics.linearRegression(x: .vector([1, 2, 3]), y: .vector([2, 4, 6])))
...
@Test func tensorValueStyleStatisticsDelegateToNamespaceAPIs() throws {
    ...
    #expect(try #require(try values.sum()).isApproximatelyEqual(to: 15))
    #expect(try #require(try values.mean()).isApproximatelyEqual(to: 3))
    #expect(try #require(try values.variance()).isApproximatelyEqual(to: 2.5))
    #expect(try #require(try values.standardDeviation()).isApproximatelyEqual(to: 2.5.squareRoot()))
    #expect(try #require(try values.percentile(95)).isApproximatelyEqual(to: 4.8))
    #expect(try #require(try values.interquartileRange()).isApproximatelyEqual(to: 2))
}

@Test func distributionAnalysisFitsContinuousDistributions() throws {
    let normal = try #require(
        try Numerica.Statistics.DistributionAnalysis.fitNormal(.vector([8, 10, 12])))
    ...
    let exponential = try #require(
        try Numerica.Statistics.DistributionAnalysis.fitExponential(.vector([0.5, 1, 1.5])))
    #expect(exponential.rate.isApproximatelyEqual(to: 1))
}

@Test func distributionAnalysisRejectsInvalidFits() throws {
    try #expect(Numerica.Statistics.DistributionAnalysis.fitNormal(.vector([2, 2, 2])) == nil)
    #expect(Numerica.Statistics.DistributionAnalysis.fitUniform(.vector([2, 2, 2])) == nil)
    try #expect(Numerica.Statistics.DistributionAnalysis.fitExponential(.vector([-1, 1, 2])) == nil)
}
